"""
Generate the 2026 Men's bracket using the balanced_rounds models
(trained on all data 2003-2025, scale_pos_weight per round).
"""
import warnings, os
import numpy as np
import pandas as pd
...
round_models, feature_cols = joblib.load(os.path.join(MODEL_DIR, 'round_models_M_balanced.joblib'))
...
print(f"\n{'='*72}")
print(f"  2026 PREDICTED CHAMPION (Balanced Rounds Model)")
...
<p class="subtitle">Generated by Balanced Rounds XGBoost model · Trained 2003–2025 (scale_pos_weight per round) · Hover games for spread/O/U</p>

<h2>MEN'S TOURNAMENT</h2>
<p class="accuracy">Win model: 81.16% accuracy (2024–2025 holdout) · Spread MAE: 9.88 pts · Total MAE: 15.55 pts</p>
...
out_path = os.path.join(BASE, 'outputs', 'bracket_balanced_visualization.html')
